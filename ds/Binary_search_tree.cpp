...
public:
	...
	void AfterDeletInsert(TreeNode * ptr);
...
void BinarySearchTree :: insertNode(TreeNode* new_node , bool check) {
	...
	if (isEmpty()) {root = new_node ; if(check) cout << "insertNode successful" << endl; return ;}
...
void BinarySearchTree:: deletNode(int d) {
	...
	while (ptr != NULL) {
		if (ptr->value < d) {
			...
			else if(prev->left == ptr) prev->left = NULL;
			else prev->right = NULL ;
			AfterDeletInsert(ptr->left);
			AfterDeletInsert(ptr->right);
...
void BinarySearchTree:: AfterDeletInsert(TreeNode * ptr){
	if(ptr == NULL) return;

	TreeNode * a = new TreeNode(ptr->value);
